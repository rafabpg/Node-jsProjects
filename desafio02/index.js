...
function checksCreateTodosUserAvailability(){
    const { userTodos} = request;

}
function checksTodoExists(){
    
}
function findUserById(){
    const { id } = request.params;
    const userTodos = users.find(user => user.id === id);
    if(!userTodos) return response.status(400).json({erro:"foi mal ta errado ai"});
    request.userTodos = userTodos;
    return next();
}
...
app.get("/todos",verifyExistUsername,checksCreateTodosUserAvailability,(request,response)=>{
    const {user} = request;
